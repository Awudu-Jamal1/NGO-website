...
function Navbar() {
    ...
    const back = colorChange?'bg-main ':''
    // const styles ="absolute flex w-full  bg-main items-center justify-between p-6 lg:px-8"
    return (  <>
     <nav className={`absolute flex w-full   items-center justify-between p-6 lg:px-8 ${back}`} aria-label="Global">
          ...
          <div className="hidden lg:flex  lg:gap-x-12">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className="hover:border-b text-sm font-semibold text-white">
                <div>{item.name}</div>
              </a>
            ))}
          </div>
